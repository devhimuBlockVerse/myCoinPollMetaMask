...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  void didChangeDependencies() {
    ...
    if (!_hasInitialized) {
      ...
      Future.microtask(() {
        final walletVM = Provider.of<WalletViewModel>(context, listen: false);
        walletVM.fetchConnectedWalletData();// Initialize everything
      });
    }
  }
  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      // App resumed (e.g. returned from MetaMask)
      debugPrint("App resumed. Re-fetching wallet data.");
      _initializeWalletData();
    }
  }
  ...
  Future<void> _initializeWalletData() async {
    ...
    try {
      ...
      if (mounted) {
        Utils.flushBarErrorMessage("Failed to fetch stage info", context);
                              ...
                              builder: (context, walletVM, _){
                                ...
                                return  BlockButton(
                                  ...
                                  onTap: walletVM.isLoading ? null : () async {
                                    ...
                                    if (walletVM.isConnected) {
                                      print("Wallet already connected. Navigating to BottomNavBar.");
                                       // walletVM.fetchConnectedWalletData();
                                      ...
                                      final connected = await walletVM.connectWallet(context);
                                      if (connected) {
                                        print("Wallet connected successfully. Fetching data...");
                                        await _initializeWalletData();
                                        if (context.mounted) {
...
//   bool isDisconnecting = false;
//
//   Future<void> _fetchAndSetConnectedWalletData() async {
//     if (!mounted) return;
//     final walletVM = Provider.of<WalletViewModel>(context, listen: false);
//
//     if (!walletVM.isConnected) {
//       print("Skipping connected wallet data fetch: Wallet is not connected.");
//       return;
//     }
//
//     try {
//       print("Fetching connected wallet specific data and stage info from connected chain...");
//
//       await walletVM.getBalance();
//       await walletVM.getCurrentStageInfo();
//
//       // readingMoreController.text = walletVM.walletAddress;
//       final stageInfo = await walletVM.getCurrentStageInfo();
//       setState(() {
//         _stageIndex = stageInfo['stageIndex'];
//         _currentECM = (stageInfo['ecmSold'] as num).toDouble();
...
//
//     } catch (e, s) {
//       print('--- ERROR ON HOMESCREEN (Connected Wallet Data) ---');
//       print('Exception details: ${e.toString()}');
//       print('Stack Trace: ${s.toString()}');
//       if (mounted) {
//         Utils.flushBarErrorMessage("Could not load connected wallet details. Please try again.", context);
//       }
//     }
//   }
//
//   // method to fetch stage data without requiring a wallet connection
//   Future<void> _fetchAndSetPublicStageData() async {
//     if (!mounted) return;
//     final walletVM = Provider.of<WalletViewModel>(context, listen: false);
//
//     try {
//       print("Attempting to fetch public stage data...");
//       final stageInfo = await walletVM.fetchStageInfo(); // Calling public method
//
//       setState(() {
//         _stageIndex = stageInfo['stageIndex'];
//         _currentECM = (stageInfo['ecmSold'] as num).toDouble();
...
//       });
//       _updatePayableAmount();
//       print("Public stage data fetched successfully.");
//     } catch (e, s) {
//       print('--- ERROR ON HOMESCREEN (Public Stage Data) ---');
//       print('Exception details: ${e.toString()}');
//       print('Stack Trace: ${s.toString()}');
//       // if (mounted) {
//       //   // Utils.flushBarErrorMessage("Could not load stage details. Please try again.", context);
...
//
//        await walletVM.init(context);
//       // fetch public stage data first
//       await _fetchAndSetPublicStageData();
//
//       // If a wallet is connected after init, fetch connected wallet specific data
//       if (walletVM.isConnected) {
//         print("Wallet was already connected or got connected during init. Fetching connected data...");
//          await _fetchAndSetConnectedWalletData();
//       }
//
//
//      });
...
//                                   //     if (connected) {
//                                   //       print("Wallet connected successfully. Fetching data...");
//                                   //       // After successful connection, update the address and fetch connected data
//                                   //       readingMoreController.text = model.walletAddress;
//                                   //       await _fetchAndSetConnectedWalletData();
//                                   //       if (context.mounted) {
//                                   //         Navigator.push(context, MaterialPageRoute(builder: (context) => const BottomNavBar()));
//                                   //       }
